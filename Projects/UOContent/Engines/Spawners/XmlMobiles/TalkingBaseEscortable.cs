...
public class TalkingBaseEscortable : TalkingBaseCreature
{
    ...
    public virtual bool SayDestinationTo(Mobile m)
    {
        ...
        if (escorter == null)
        {
            Say($"I am looking to go to {(dest.Name == "Ocllo" && m.Map == Map.Trammel ? "Haven" : dest.Name)}, will you take me?");
            return true;
        }

        if (escorter == m)
        {
            Say($"Lead on! Payment will be made when we arrive in {(dest.Name == "Ocllo" && m.Map == Map.Trammel ? "Haven" : dest.Name)}.");
            return true;
        }
    ...
    private static Dictionary<Mobile, TalkingBaseEscortable> m_EscortTable = new();

    public static Dictionary<Mobile, TalkingBaseEscortable> EscortTable => m_EscortTable;
    ...
    public virtual bool AcceptEscorter(Mobile m)
    {
        ...
        m_EscortTable.TryGetValue(m, out var escortable);
        ...
        if (m is PlayerMobile mobile && mobile.LastEscortTime + m_EscortDelay >= DateTime.Now)
        {
            ...
            Say($"You must rest {minutes} minute{(minutes == 1 ? "" : "s")} before we set out on this journey.");
            return false;
        }
    ...
    public virtual bool CheckAtDestination()
    {
        ...
        if (dest.Contains(Location))
        {
            ...
            if (pm != null)
            {
                ...
                if (virtues.CompassionGains > 0 && DateTime.Now > virtues.NextCompassionDay)
                {
                    virtues.NextCompassionDay = DateTime.MinValue;
                    virtues.CompassionGains = 0;
                }

                if (virtues.CompassionGains >= 5) // have already gained 5 points in one day, can gain no more
                {
                    pm.SendLocalizedMessage(1053004); // You must wait about a day before you can gain in compassion again.
                }
                else if (VirtueSystem.Award(pm, VirtueName.Compassion, 1, ref gainedPath))
                {
                    if (gainedPath)
                    {
                    ...
                    virtues.NextCompassionDay = DateTime.Now + TimeSpan.FromDays(1.0); // in one day CompassionGains gets reset to 0
                    ++virtues.CompassionGains;
                }
                else
                {
