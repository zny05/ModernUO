...
public class XmlSpawner : Item, ISpawner
{
    ...
    private const int defKillReset = 1;
    ...
    public List<SpawnObject> m_SpawnObjects = []; // List of objects to spawn
    ...
    internal List<BaseXmlSpawner.KeywordTag> m_KeywordTagList = [];
    ...
    private List<Static> m_ShowBoundsItems = [];
...
#pragma warning restore CS0649, CS0414
    ...
    public bool DisableGlobalAutoReset { get => m_DisableGlobalAutoReset;
    ...
    public int WalkingRange => HomeRange;

    public Rectangle3D SpawnBounds => new(m_X, m_Y, Z, m_Width, m_Height, 0);

    public bool IsInSpawnBounds(Point3D location)
    {
        return SpawnBounds == default || SpawnBounds.Contains(location);
    }

    public virtual void GetSpawnProperties(ISpawnable spawn, IPropertyList list)
    { }
    ...
    public override void GetProperties(IPropertyList list)
    {
        ...
        if (m_SpawnObjects != null)
        {
            for (int i = 0; i < nlist_items && i < m_SpawnObjects.Count; ++i)
            {
                ...
                list.Add(1060658 + (6 - nlist_items) + i, $"{typename}\t{m_SpawnObjects[i].SpawnedObjects.Count}");
            }
        }
    }
    ...
    public bool SomeOneHasGumpOpen
    {
        get
        {
            ...
            {
                var m = ns.Mobile;
                if (m != null && m.HasGump<XmlSpawnerGump>())
                {
                    return true;
                }
            }
        ...
        else
        {
            ...
            try
            {
                ...
                if (o is Mobile mobile)
                {
                    ...
                    BaseXmlSpawner.ApplyObjectStringProperties(null, substitutedtypeName, mobile, trigmob, attachedto, out status_str);
                }
                else
                if (o is Item item)
                {
    ...
    private class MovementTimer : Timer
    {
        ...
        public MovementTimer(XmlSpawner spawner, TimeSpan delay) : base(delay) => m_Spawner = spawner;
            ...
            case "ChangeCommand":
                {
                    ...
                    Timer.DelayCall(TimeSpan.Zero, DelayedAssignSettings, argname, value);
                    break;
                }
            default:
                {
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            foreach (SpawnObject so in spawner.m_SpawnObjects)
            {
                for (int x = 0; x < so.SpawnedObjects.Count; x++)
                {
                    ...
                    if (o == targeted)
                    {
                        from.SendMessage($"{spawner.X}, {spawner.Y}, {spawner.Z}");
                        ...
                        if (m_e.GetString(0) == "send")
                        {
                            ...
                            if (spawner.Parent == null)
                            {
                                ...
                                if (o is Mobile mobile1)
                                {
                                    mobile1.Location = new Point3D(spawner.Location);
                                    mobile1.Map = spawner.Map;
                                }
                            }
                            else
                            {
    ...
    public static void XmlUnLoadFromStream(Stream fs, string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
    {
        ...
        if (from != null)
        {
            from.SendMessage($"{spawners_deleted}/{TotalCount} spawner(s) were unloaded using file {filename} [Trammel={TrammelCount}, Felucca={FeluccaCount}, Ilshenar={IlshenarCount}, Malas={MalasCount}, Tokuno={TokunoCount}, Other={OtherCount}].");
        }
    ...
    public static void XmlImportMap(string filename, Mobile from, out int processedmaps, out int processedspawners)
    {
        ...
        if (Directory.Exists(filename))
        {
            ...
            from.SendMessage($"Imported a total of {total_processed_maps} .map files and {total_processed_spawners} spawners from directory {filename}");
            processedmaps = total_processed_maps;
            processedspawners = total_processed_spawners;
        }
        else
            ...
            if (!badspawn && totalspawns > 0)
            {
                ...
                for (int k = 0; k < 6; k++)
                {
                    ...
                    for (int i = 0; i < typenames[k].Length; i++)
                    {
                        ...
                        Type type = AssemblyHandler.FindTypeByName(typenames[k][i]);
                ...
                XmlSpawner spawner = new XmlSpawner(SpawnId, x, y, 0, 0, spawnername, totalmaxcount,
                    ...
                    TimeSpan.FromHours(0), null, null);
        ...
        if (args.Length > 0 && args[0] == "*")
        {
            ...
            if (!badspawn && typenames.Length > 0)
            {
                ...
                for (int i = 0; i < typenames.Length; i++)
                {
                    ...
                    // check the type to see if there are vendors on it
                    Type type = AssemblyHandler.FindTypeByName(typenames[i]);
    ...
    private static void ImportSpawner(XmlElement node, Mobile from)
    {
        ...
        for (int i = 0; i < creaturesName.Count; i++)
        {
            ...
            // check the type to see if there are vendors on it
            Type type = AssemblyHandler.FindTypeByName(creaturesName[i]);
        ...
        XmlSpawner spawner = new XmlSpawner(SpawnId, location.X, location.Y, spawnwidth, spawnwidth, name, count,
            ...
            TimeSpan.FromMinutes(0), null, null, null, null, null,
            null, null, null, null, 1, null, group, defTODMode, defKillReset, false, -1, null, false, false, false, null, defDespawnTime, null, null);
    ...
    public static void XmlLoadFromFile(string filename, string SpawnerPrefix, Mobile from, Point3D fromloc, Map frommap, bool loadrelative, int maxrange, bool loadnew, out int processedmaps, out int processedspawners)
    {
        ...
        else if (Directory.Exists(filename))
        {
            ...
            if (from != null)
            {
                from.SendMessage($"Loaded a total of {total_processed_maps} .xml files and {total_processed_spawners} spawners from directory {filename}");
            }
                    ...
                    if (string.IsNullOrEmpty(SpawnerPrefix) || SpawnName.StartsWith(SpawnerPrefix))
                    {
                        ...
                        if (!bad_spawner)
                        {
                            ...
                            XmlSpawner TheSpawn = new XmlSpawner(SpawnId, SpawnX, SpawnY, SpawnWidth, SpawnHeight, SpawnName, SpawnMaxCount,
                                ...
                                SpawnRegionName, SpawnAllowGhost, SpawnAllowNPC, SpawnSpawnOnTrigger, SpawnConfigFile, SpawnDespawnTime, SpawnSkillTrigger, SpawnWaypoint)
                            {
    ...
    public static bool SaveSpawnList(List<XmlSpawner> savelist, Stream stream) => SaveSpawnList(null, savelist, null, stream, false, false);
    ...
    public static void XmlMake_OnCommand(CommandEventArgs e)
    {
        ...
        if (e.Arguments.Length > 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (e.Arguments.Length > 2)
                {
                    Spawner x = new Spawner(10, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(1), 0, spawnedNames: new[] { e.Arguments[1] });
        ...
        for (int i = 0; i < MaxTraces; i++)
        {
            if (_traceCount[i] > 0)
            {
                ...
                Console.WriteLine("{0} ({4}) {1,21} / {2} calls = {3:####.####} ms/call, {5:p3}",
                    i, _traceTotal[i], _traceCount[i], _traceTotal[i].TotalMilliseconds / _traceCount[i],
                    _traceName[i], load);
            }
        }
    }
        ...
        : base(BaseItemId)
    {
        ...
        InitSpawn(0, 0, m_Width, m_Height, string.Empty, 0, defMinDelay, defMaxDelay, defDuration,
            defProximityRange, defProximityTriggerSound, defAmount, defTeam, defHomeRange, defRelativeHome, [], defMinRefractory, defMaxRefractory,
            defTODStart, defTODEnd, null, null, null, null, null, null, null, null, null, defTriggerProbability, null, defIsGroup, defTODMode,
            defKillReset, false, -1, null, false, false, false, null, defDespawnTime, null, null);
    }
    ...
    public XmlSpawner(Guid uniqueId, int x, int y, int width, int height, string name, int maxCount, TimeSpan minDelay, TimeSpan maxDelay, TimeSpan duration,
        ...
        bool allowghost, bool allownpc, bool spawnontrigger, string configfile, TimeSpan despawnTime, string skillTrigger, WayPoint wayPoint)
        : base(BaseItemId)
    {
        ...
        InitSpawn(x, y, width, height, name, maxCount, minDelay, maxDelay, duration,
            ...
            despawnTime, skillTrigger, wayPoint);
    }
    ...
    public void InitSpawn(int x, int y, int width, int height, string name, int maxCount, TimeSpan minDelay, TimeSpan maxDelay, TimeSpan duration,
        ...
        string configfile, TimeSpan despawnTime, string skillTrigger, WayPoint wayPoint)
    {
    ...
    public void Defrag(bool killtest)
    {
        ...
        List<Item> deleteilist = [];
        List<Mobile> deletemlist = [];
        foreach (SpawnObject so in m_SpawnObjects)
        {
            for (int x = 0; x < so.SpawnedObjects.Count; x++)
            {
                ...
                else if (o is Mobile mobile)
                {
                    ...
                    {
                        //m.Delete();
                        deletemlist.Add(mobile);
                        despawned = true;
                    }

                    if (mobile.Deleted || despawned)
                    {
                        // Remove the delete mobile from the list
                        so.SpawnedObjects.Remove(mobile);
                        x--;
                        removed = true;
                        // if sequential spawning is active and the RestrictKillsToSubgroup flag is set, then check to see if
                        // the object is in the current subgroup before adding to the total
                    ...
                    else if (mobile is BaseCreature creature)
                    {
                        ...
                        if (creature.Controlled || creature.IsStabled || creature.Owners != null && creature.Owners.Count > 0)
                        {
                            so.SpawnedObjects.Remove(mobile);
                            x--;
                            removed = true;
    ...
    public void ClearGOTOTags()
    {
        ...
        List<BaseXmlSpawner.KeywordTag> ToDelete = [];
        foreach (SpawnObject so in m_SpawnObjects)
        {
            for (int x = 0; x < so.SpawnedObjects.Count; x++)
            {
    ...
    public void OnTick()
    {
        ...
        if (!OnHold)
        {
            ...
            if (m_ProximityRange >= 0 && CanSpawn)
            {
                ...
                while (queue.Count > 0)
                {
                    CheckTriggers(queue.Dequeue(), null, true);
                }
            }
    ...
    // spawn an individual entry by index up to count times
    public bool Spawn(int index, bool smartspawn, int count, byte loops) => Spawn(index, smartspawn, count, false, loops);

    // spawn an individual entry by index up to count times
    public bool Spawn(int index, bool smartspawn, int count, bool ignoreloopprotection, byte loops) => Spawn(index, smartspawn, count, -1, Point3D.Zero, ignoreloopprotection, loops);
    ...
    // spawn an individual entry by index
    public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord, byte loops) => Spawn(index, smartspawn, packrange, packcoord, false, loops);
            ...
            if (type != null && !(Parent != null && (type == typeof(Mobile) || type.IsSubclassOf(typeof(Mobile)))))
            {
                ...
                try
                {
                    if (o is Mobile mob)
                    {
                        ...
                        TheSpawn.SpawnedObjects.Add(mob);

                        mob.Spawner = this;

                        var loc = GetSpawnPosition(requiresurface, packrange, packcoord, spawnpositioning, mob);
                        ...
                        if (mob is BaseCreature mobile)
                        {
                            mobile.RangeHome = m_HomeRange;
                            mobile.CurrentWayPoint = m_WayPoint;
                            ...
                            mobile.Home = m_HomeRangeIsRelative ? mobile.Location : Location;
                        }
                        ...
                        BaseXmlSpawner.ApplyObjectStringProperties(this, substitutedtypeName, mob, m_mob_who_triggered, this, out status_str);
    ...
    public bool SpawnSubGroup(int sgroup, byte loops) => SpawnSubGroup(sgroup, false, loops);

    public bool SpawnSubGroup(int sgroup, bool smartspawn, byte loops) => SpawnSubGroup(sgroup, false, false, loops);
    ...
    public void BringToHome()
    {
        ...
        foreach (SpawnObject so in m_SpawnObjects)
        {
            for (int i = 0; i < so.SpawnedObjects.Count; ++i)
            {
                ...
                {
                    mobile.Map = Map;
                    mobile.Location = new Point3D(Location);
                }
            ...
            if (wayargs != null && wayargs.Length > 0)
            {
                ...
                if (wayargs[0] == "SERIAL")
                {
                    ...
                    if (wayargs.Length > 1)
                    {
                        ...
                        try
                        {
                            sernum = (uint)Convert.ToUInt64(wayargs[1][2..], 16);
                            IEntity e = World.FindEntity((Serial)sernum);
    ...
    private static bool HasTileSurface(Map map, int X, int Y, int Z)
    {
        ...
        foreach (StaticTile o in map.Tiles.GetStaticAndMultiTiles(X, Y))
        {
            StaticTile i = o;
    ...
    public bool CanFit(int x, int y, int z, int height, bool checkBlocksFit, bool checkMobiles, bool requireSurface, Mobile mob)
    {
        ...
        map.GetAverageZ(x, y, out var lowZ, out var avgZ, out var topZ);
        ...
        // Check items at this location using ModernUO API
        ...
        if (checkMobiles)
        {
            // Check mobiles at this location using ModernUO API
            foreach (Mobile m in map.GetMobilesInRange(new Point3D(x, y, z), 0))
            {
    ...
    public bool CanSpawnMobile(int x, int y, int z, Mobile mob)
    {
        ...
        return Region.Find(new Point3D(x, y, z), Map).AllowSpawn() && Map.CanFit(x, y, z, 16);
    }

    public static bool HasRegionPoints(Region r) => r != null && r.Area.Length > 0;
    ...
    private void FindTileLocations(ref List<Point3D> locations, Map map, int startx, int starty, int width, int height, List<int> includetilelist, List<int> excludetilelist, TileFlag tileflag, bool checkitems, int spawnerZ)
    {
        ...
        for (int x = startx; x <= startx + width; x++)
        {
            for (int y = starty; y <= starty + height; y++)
            {
                ...
                foreach (var stile in map.Tiles.GetStaticAndMultiTiles(x, y))
                {
    ...
    public Point3D GetSpawnPosition(bool requiresurface) =>
        // no pack spawning
        GetSpawnPosition(requiresurface, -1, Point3D.Zero, null, null);
                ...
                if (delete_this_entry)
                {
                    ...
                    if (from != null)
                    {
                        ...
                        CommandLogging.WriteLine(
                            from,
                            $"{from.AccessLevel} {CommandLogging.Format(from)} removed from XmlSpawner {Serial} '{Name}' [{loc.X}, {loc.Y}] ({Map}) : {SpawnObjectName}"
                        );
                    }
                }
            }
    ...
    public static object CreateObject(Type type, string itemtypestring) => CreateObject(type, itemtypestring, true);
    ...
    public static object CreateObject(Type type, string itemtypestring, bool requireConstructible)
    {
        ...
        return CreateObject(type, typewordargs, requireConstructible);
    }

    public static object CreateObject(Type type, string[] typewordargs, bool requireConstructible)
    {
        if (type == null)
        {
            return null;
        ...
        for (int i = 0; i < ctors.Length; ++i)
        {
            ...
            // if requireConstructible is true, then allow either condition
#if (RESTRICTConstructible)
			   if (!(requireConstructible && Add.IsConstructible(ctor,requester)))
					continue;
#else
            if (!(requireConstructible && IsConstructible(ctor)))
            {
                continue;
            }
#endif
    ...
    private class InternalTimer : Timer
    {
        ...
        public InternalTimer(XmlSpawner spawner, TimeSpan delay) : base(delay) => m_spawner = spawner;
    ...
    private class SpawnerTimer : Timer
    {
        ...
        public SpawnerTimer(XmlSpawner spawner, TimeSpan delay) : base(delay) => m_Spawner = spawner;
    ...
    private class InternalTimer3 : Timer
    {
        ...
        public InternalTimer3(XmlSpawner spawner, TimeSpan delay) : base(delay) => m_spawner = spawner;
    ...
    public override void Serialize(IGenericWriter writer)
    {
        ...
        {
            writer.Write(true);
            writer.Write(m_ShowBoundsItems);
        }
        else
        {
        ...
        // Version 23 - SmartSpawning removed, write false for backwards compatibility
    ...
    public override void Deserialize(IGenericReader reader)
    {
        ...
        switch (version)
        {
            case 33: // Removes smart spawning
            case 32:
            case 31:
                {
            ...
            case 24:
                {
                    ...
                    if (hasitems)
                    {
                        m_ShowBoundsItems = reader.ReadEntityList<Static>();
                    }
                    goto case 23;
                }
            case 23:
                {
                    if (version < 33)
                    {
                        reader.ReadBool(); // was IsInactivated
                        reader.ReadBool(); // was SmartSpawning
                    }
            ...
            case 22:
                {
                    ...
                    m_mob_who_triggered = reader.ReadEntity<Mobile>();
                    goto case 21;
                }
            case 21:
                {
            ...
            case 10:
                {
                    m_SetPropertyItem = reader.ReadEntity<Item>();
                    goto case 9;
                }
            case 9:
                {
            ...
            case 5:
                {
                    ...
                    m_ObjectPropertyItem = reader.ReadEntity<Item>();
            ...
            case 3:
                {
                    m_ShowContainerStatic = reader.ReadEntity<Static>() ;
                    goto case 2;
                }
            case 2:
                {
            ...
            case 0:
                {
                    ...
                    m_WayPoint = reader.ReadEntity<WayPoint>();
        ...
        {
            Timer.DelayCall(delegate { if (!Deleted && m_RegionName != null)
                {
    ...
    public class SpawnObject
    {
        ...
        public SpawnObject(Mobile from, XmlSpawner spawner, string name, int maxamount)
        {
            ...
            if (from != null && spawner != null)
            {
                ...
                if (!found)
                {
                    ...
                    CommandLogging.WriteLine(
                        from,
                        $"{from.AccessLevel} {CommandLogging.Format(from)} added to XmlSpawner {spawner.Serial} '{spawner.Name}' [{loc.X}, {loc.Y}] ({spawner.Map}) : {name}"
                    );
                }
            }
        ...
        internal static SpawnObject[] LoadSpawnObjectsFromString(string ObjectList)
        {
            ...
            List<SpawnObject> NewSpawnObjects = [];
