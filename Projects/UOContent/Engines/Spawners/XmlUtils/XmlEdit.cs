...
public class XmlEditDialogGump : Gump
{
    ...
    private class ListSorter : IComparer<XmlDialog.SpeechEntry>
    {
        ...
        {
            int xn = x?.EntryNumber ?? 0;
            int yn = y?.EntryNumber ?? 0;
    ...
    public override void OnResponse(NetState state, in RelayInfo info)
    {
        ...
        if (m_Dialog != null)
        {
            ...
            if (tr != null)
            {
                try
                {
                    m_Dialog.ProximityRange = int.Parse(tr);
                }
                catch{}
            }
            ...
            if (tr != null)
            {
                try
                {
                    m_Dialog.ResetTime = TimeSpan.Parse(tr);
                }
                catch{}
            }
            ...
            if (tr != null)
            {
                try
                {
                    m_Dialog.SpeechPace = int.Parse(tr);
                }
                catch{}
            }
            ...
            {
                if (!string.IsNullOrEmpty(tr?.Trim()))
                {
                    m_Dialog.TriggerOnCarried = tr;
                }
                else
                {
            ...
            if (tr != null && (m_Dialog.NoTriggerOnCarried == null || m_Dialog.NoTriggerOnCarried.Length < 230))
            {
                if (!string.IsNullOrEmpty(tr?.Trim()))
                {
                    m_Dialog.NoTriggerOnCarried = tr;
                }
                else
                {
        ...
        if (m_SearchList != null && Selected >= 0 && Selected + DisplayFrom >= 0 && Selected + DisplayFrom < m_SearchList.Count)
        {
            ...
            if (tr != null)
            {
                try
                {
                    entry.EntryNumber = int.Parse(tr);
                }
                catch {}
            }
            ...
            default:
                {
                    ...
                    if (info.ButtonID >= 5000 && info.ButtonID < 5100)
                    {
                        ...
                        {
                            m_Dialog.m_TextEntryBook = new List<Item>();
                        }
