...
public class TextEntryGump : Gump
{
    ...
    public override void OnResponse(NetState state, in RelayInfo info)
    {
        ...
        if (edit_entry)
        {
            ...
            string oldtext = entry;
            // get the new text
            entry = info.GetTextEntry(2);
            string newtext = entry;
            // make the substitution
            entry = info.GetTextEntry(0);
            string origtext = entry;
        ...
        if (update_entry)
        {
            ...
            if (m_index < m_Spawner.SpawnObjects.Length)
            {
                m_Spawner.SpawnObjects[m_index].TypeName = entry;
            }
            else
            {
                // Update the creature list
...
public class XmlSpawnerGump : Gump
{
    ...
    {
        List<XmlSpawner.SpawnObject> SpawnObjects = new List<XmlSpawner.SpawnObject>();
                ...
                if (str.Length > 0)
                {
                    ...
                    if (i < m_Spawner.SpawnObjects.Length)
                    {
...
#endif
                        {
                            ...
                            {
                                var loc = m_Spawner.GetWorldLocation();
                                CommandLogging.WriteLine(
                                    from,
                                    $"{from.AccessLevel} {CommandLogging.Format(from)} changed XmlSpawner {m_Spawner.Serial} '{m_Spawner.Name}' [{loc.X}, {loc.Y}] ({m_Spawner.Map}) : {m_Spawner.SpawnObjects[i].TypeName} to {str}"
                                );
                            }
    ...
    public static void RefreshSpawnerGumps(Mobile from)
    {
        ...
        List<XmlSpawnerGump> refresh = new List<XmlSpawnerGump>();
    ...
    public override void OnResponse(NetState state, in RelayInfo info)
    {
        ...
        for (int i = 0; i < m_Spawner.SpawnObjects.Length; i++)
        {
            ...
            {
                int maxval = 0;
                try { maxval = Convert.ToInt32(temcnt, 10); }
                catch (Exception e) { Diagnostics.ExceptionLogging.LogException(e); }
                if (maxval < 0)
                {
                    maxval = 0;
            ...
            if (m_ShowGump > 0)
            {
                ...
                {
                    int grpval = 0;
                    try { grpval = Convert.ToInt32(tegrp, 10); }
                    catch (Exception e) { Diagnostics.ExceptionLogging.LogException(e); }
                    if (grpval < 0)
                    {
                        grpval = 0;
            ...
            if (m_ShowGump > 1)
            {
                ...
                if (subgroupindex >= 0 && subgroupindex < m_Spawner.SpawnObjects.Length)
                {
                    ...
                    if (tegrp != null && tegrp.Length > 0)
                    {
                        double grpval = 0;
                        try { grpval = Convert.ToDouble(tegrp); }
                        catch (Exception e) { Diagnostics.ExceptionLogging.LogException(e); }
                        if (grpval < 0)
                        {
                            grpval = 0;
        ...
        {
            int maxval = 0;
            try { maxval = Convert.ToInt32(temax, 10); }
            catch (Exception e) { Diagnostics.ExceptionLogging.LogException(e); }
            if (maxval < 0)
            {
                maxval = 0;
            ...
            default:
                {
                    ...
                    else if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxSpawnEntries)
                    {
                        ...
                        else
                        {
                            ...
                            List<object> types = XmlPartialCategorizedAddGump.Match(categorystring);
