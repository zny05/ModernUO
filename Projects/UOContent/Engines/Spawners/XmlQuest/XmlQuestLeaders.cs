...
public class XmlQuestLeaders
{
    ...
    public static void QuestLeaderboardSave_OnCommand(CommandEventArgs e)
    {
        ...
        {
            e.Mobile.SendMessage($"Quest Leaderboard is saving to {m_QuestLeaderboardFile} every {m_QuestLeaderboardSaveInterval.TotalMinutes} minutes. Nranks = {m_QuestLeaderboardSaveRanks}");
        }
        else
        {
    ...
    public static void QuestLBSDeserialize(IGenericReader reader)
    {
    ...
    public class TopQuestPlayersGump : Gump
    {
        ...
        public override void OnResponse(NetState state, in RelayInfo info)
        {
            ...
            if (m_attachment != null)
            {
                ...
                if (entry != null)
                {
                    m_attachment.guildFilter = entry;
                }

                entry = info.GetTextEntry(100);
                if (entry != null)
                {
                    m_attachment.nameFilter = entry;
                }
            }
