...
public class XmlPlayerQuestGump : Gump
{
    ...
    public override void OnResponse(NetState sender, in RelayInfo info)
    {
        ...
        var tr = info.GetTextEntry(100); // quest name
        if (tr != null)
        {
            m_QuestItem.Name = tr.Trim();
        }

        tr = info.GetTextEntry(102); // title
        if (tr != null)
        {
            m_QuestItem.TitleString  = tr.Trim();
        }

        tr = info.GetTextEntry(103); // notestring
        if (tr != null)
        {
            m_QuestItem.NoteString = tr;
        }

        tr = info.GetTextEntry(200); // objectives
        if (tr != null)
        {
            m_QuestItem.Objective1 = tr.Trim();
        }

        tr = info.GetTextEntry(201);
        if (tr != null)
        {
            m_QuestItem.Objective2 = tr.Trim();
        }

        tr = info.GetTextEntry(202);
        if (tr != null)
        {
            m_QuestItem.Objective3 = tr.Trim();
        }

        tr = info.GetTextEntry(203);
        if (tr != null)
        {
            m_QuestItem.Objective4 = tr.Trim();
        }

        tr = info.GetTextEntry(204);
        if (tr != null)
        {
            m_QuestItem.Objective5 = tr.Trim();
        }

        tr = info.GetTextEntry(205);
        if (tr != null && tr.Length > 0) // descriptions
        {
            m_QuestItem.Description1 = tr.Trim();
        }
        else
        {
        ...
        {
            m_QuestItem.Description2 = tr.Trim();
        }
        else
        {
        ...
        if (tr != null && tr.Length > 0)
        {
            m_QuestItem.Description3 = tr.Trim();
        }
        else
        {
        ...
        if (tr != null && tr.Length > 0)
        {
            m_QuestItem.Description4 = tr.Trim();
        }
        else
        {
        ...
        if (tr != null && tr.Length > 0)
        {
            m_QuestItem.Description5 = tr.Trim();
        }
        else
        {
        ...
        if (tr != null && tr.Length > 0)
        {
            try
            {
                m_QuestItem.Expiration = double.Parse(tr.Trim());
            }
            catch
            {
            }
    ...
    private class RewardTarget : Target
    {
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
                ...
                !(o is IXmlQuest))
            {
                ...
                if (!o.Deleted && o.RootParent == m_QuestItem.Owner)
                {
