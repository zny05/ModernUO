...
public abstract class XmlQuestHolder : Container, IXmlQuest
{
    ...
    public List<XmlQuest.JournalEntry> Journal { get => m_Journal;
        set => m_Journal = value;
    }
    private static char[] colondelim = [':'];
            ...
            else
            {
                ...
                if (holder != null)
                {
                    if (notify)
                    {
                        ...
                        holder.SendMessage(JournalNotifyColor, $"Journal entry '{entryID}' of quest '{Name}' has been modified.");
                    }
                    if (echo)
                    {
                        // echo the journal text to the player holding the questholder
                        holder.SendMessage(JournalEchoColor, entryText);
                    }
                }
            }
        }
        else
        {
            ...
            if (entryText != null && entryText.Length != 0)
            {
                ...
                if (holder != null)
                {
                    ...
                    {
                        // notify the player holding the questholder
                        holder.SendMessage(JournalNotifyColor, $"Journal entry '{entryID}' has been added to quest '{Name}'.");
                    }
                    if (echo)
                    {
                        // echo the journal text to the player holding the questholder
    ...
    private void ReturnReward()
    {
        if (m_RewardItem != null)
        {
            ...
            if (PlayerMade && Creator != null && !Creator.Deleted)
            {
                ...
                if (returned)
                {
                    Creator.SendMessage($"Your reward {m_RewardItem.GetType().Name} was returned from quest {Name}");
                    //AddMobileWeight(Creator, m_RewardItem);
                }
                else
                {
                    Creator.SendMessage($"Attempted to return reward {m_RewardItem.GetType().Name} from quest {Name} : containers full.");
                }
            }
            else
            {
    ...
    private void PackItemsMovable(Container pack, bool canmove)
    {
        ...
        UnHideRewards();
        foreach (var i in pack.FindItems())
        {
            i.Movable = canmove;
        }
    }
    ...
    private void CheckOwnerFlag()
    {
        if (Owner != null && !Owner.Deleted)
        {
            ...
            List<IXmlQuest> list = XmlQuest.FindXmlQuest(Owner);
