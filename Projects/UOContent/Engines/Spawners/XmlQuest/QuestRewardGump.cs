                    ...
                    if (info.ButtonID >= 1000)
                    {
                        ...
                        if (selection < Rewards.Count)
                        {
                            ...
                            if (XmlQuestPoints.HasCredits(from, r.Cost))
                            {
                                ...
                                {
                                    from.SendMessage(33, $"unable to create {r.RewardType.Name}.");
                                    received = false;
                                }
                                ...
                                if (received)
                                {
                                    ...
                                    from.SendMessage($"You have purchased {r.Name} for {r.Cost} credits.");
                                }
                            }
                            else
                            {
                                from.SendMessage($"Insufficient Credits for {r.Name}.");
                            }
