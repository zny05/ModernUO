...
public abstract class XmlQuest
{
    ...
    private static void ReturnCollected(IXmlQuest quest, Item item)
    {
        ...
        if (quest != null && quest.PlayerMade && quest.Creator != null && !quest.Creator.Deleted && quest.Creator != quest.Owner)
        {
            ...
            quest.Creator.SendMessage($"You receive {item.GetType().Name} from quest {quest.Name}");
        }
        else
        {
            // just delete it
    ...
    public static List<IXmlQuest> FindXmlQuest(PlayerMobile from)
    {
        ...
        List<IXmlQuest> itemlist = new List<IXmlQuest>();

        for (int i = 0; i < packlist.Count; ++i)
        {
            Item item = packlist[i];

            if (item != null && !item.Deleted && item is IXmlQuest quest)
            {
                //found it
            ...
            if (item is XmlQuestBook book)
            {
                ...
                {
                    if (xi != null && !xi.Deleted && xi is IXmlQuest questItem)
                    {
                        itemlist.Add(questItem);
                    }
                }
            }
        }
        ...
        if (held != null && !held.Deleted && held is IXmlQuest heldQuest)
        {
            //found it
            // add the item to the list
            itemlist.Add(heldQuest);
        }
        return itemlist;
    }
    ...
    public static bool RegisterGive(Mobile from, Mobile to, Item item)
    {
        ...
        if (item != null && !item.Deleted && from is PlayerMobile mobile)
        {
            ...
            List<IXmlQuest> questlist = FindXmlQuest(mobile);
            if (questlist != null)
            {
    ...
    public static void CheckKilled(Mobile m_killed, Mobile m_killer, Mobile member)
    {
        ...
        List<IXmlQuest> mobitems = FindXmlQuest(mobile);
    ...
    public static HashSet<Map.Sector> VisitSectorList = new();
    ...
    public static void RegisterMove(PlayerMobile m_player)
    {
        ...
        Map.Sector newSector = m_player.Map.GetSector(m_player.Location);
    ...
    public static void CheckVisited(PlayerMobile m_player)
    {
        ...
        // search the player for IXmlQuest objects
        List<IXmlQuest> mobitems = FindXmlQuest(m_player);
    ...
    public static bool VerifyObjective(string[] arglist, out string status_str)
    {
        ...
        switch (arglist[0])
        {
            ...
            case "KILL":
                {
                    CheckArgList(arglist, 2, null, out typestr, out _, out _, out status_str);
                    if (arglist.Length > 1)
                    {
            ...
            case "GIVENAMED":
                {
                    CheckArgList(arglist, 3, null, out typestr, out _, out _, out status_str);
                    if (arglist.Length < 1)
                    {
                        status_str = $"{arglist[0]}missing args";
                        return false;
