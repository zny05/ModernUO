...
public class BaseXmlSpawner
{
    ...
    {
        public List<PropertyInfo> plist = []; // hold propertyinfo list
        public Type t;
    }
    ...
    private static bool IsParsable(Type t) => t == typeofTimeSpan;
    ...
    public static string ConstructFromString(PropertyInfo p, Type type, object obj, string value, ref object constructed)
    {
        ...
        else if (IsCustomEnum(type))
        {
            try
            {
                MethodInfo info = p.PropertyType.GetMethod("Parse", [typeof(string)]);
                if (info != null)
                {
                    toSet = info.Invoke(null, [value]);
                }
                ...
                else
                {
                    if (IsValuemodKeyword(value_keywordargs[0]))
                    {
                        ...
                        else if (kw == valuemodKeyword.PLAYERSINRANGE)
                        {
                            ...
                            {
                                foreach (var p in item.GetMobilesInRange<PlayerMobile>(range))
                                {
                            ...
                            {
                                foreach (var p in mobile.GetMobilesInRange<PlayerMobile>(range))
                                {
                                    if (p.AccessLevel == AccessLevel.Player)
                                    {
                                        nplayers++;
    ...
    public static void AddToRecentSpawnerSearchList(XmlSpawner spawner, XmlSpawner target)
    {
        ...
        {
            spawner.RecentSpawnerSearchList = [];
        }
    ...
    public static void AddToRecentMobileSearchList(XmlSpawner spawner, Mobile target)
    {
        ...
        {
            spawner.RecentMobileSearchList = [];
        }
    ...
    public static List<Item> GetItems(Region r)
    {
        ...
        // Iterate through all items in the world and check if they're in the region
        // This is less efficient but works with ModernUO's API
        foreach (Item item in World.Items.Values)
        {
            if (item != null && !item.Deleted && item.Map == r.Map && r.Contains(item.Location))
            {
                list.Add(item);
            }
        }
