...
public class XmlSetCustomEnumGump : XmlSetListOptionGump
{
    ...
    public XmlSetCustomEnumGump(PropertyInfo prop, Mobile mobile, object o, Stack<StackEntry> stack, int propspage, List<object> list, string[] names) : base(prop, mobile, o, stack, propspage, list, names, null)
    {
        m_Names = names;
    }
    ...
    public override void OnResponse(NetState sender, in RelayInfo relayInfo)
    {
        ...
        if (index >= 0 && index < m_Names.Length)
        {
            try
            {
                MethodInfo info = m_Property.PropertyType.GetMethod("Parse", [typeof(string)]);

                CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, m_Names[index]);

                if (info != null)
                {
                    m_Property.SetValue(m_Object, info.Invoke(null, [m_Names[index]]), null);
                }
