...
public class XmlDialog : XmlAttachment
{
    ...
    private List<SpeechEntry> m_SpeechEntries = new();  // contains the list of speech entries
    ...
    public List<Item> m_TextEntryBook;
    ...
    private SpeechEntry FindMatchingKeyword(Mobile from, string keyword, int currententryid)
    {
        ...
        foreach (SpeechEntry s in m_SpeechEntries)
        {
            ...
            if ((CheckDependsOn(s, -1) || CheckDependsOn(s, -2)) && !IsActive || CheckDependsOn(s, currententryid) && (IsActive || currententryid == 0))
            {
                ...
                if (s.Condition != null)
                {
                    if (!BaseXmlSpawner.CheckPropertyString(null, this, s.Condition, out _))
                    {
                        continue;
                    }
                }
    ...
    private void ExecuteGump(Mobile mob, string gumpstring)
    {
        ...
        if (typeName == "GUMP")
        {
            BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName, mob, map, out status_str);
    ...
    private void ExecuteAction(Mobile mob, string action)
    {
        ...
        {
            BaseXmlSpawner.SpawnTypeKeyword(AttachedTo, TheSpawn, typeName, substitutedtypeName, mob, map, out status_str);
        }
        else
        {
        ...
        if (matchentry != null)
        {
            ...
            if (text != null)
            {
                ...
                if (AttachedTo is Mobile mobile)
                {
                    ...
                    ((Mobile)AttachedTo).DoSpeech(text, [], CurrentEntry.SpeechStyle, speechhue);
    ...
    public void DoLoadNPC(Mobile from, string filename)
    {
        ...
        if (File.Exists(dirname))
        {
            ...
            if (fileerror)
            {
                ...
                {
                    from.SendMessage(33, $"Error reading npc file {dirname}");
                }
            ...
            if (ds.Tables != null && ds.Tables.Count > 0)
            {
                ...
                if (from != null && !from.Deleted)
                {
                    from.SendMessage($"Loaded npc from file {dirname}");
                }
            }
            else
            {
                if (from != null && !from.Deleted)
                {
                    from.SendMessage(33, $"No npc data found in: {dirname}");
                }
            }
        }
        else
    ...
    public bool SaveFile(Mobile from, DataSet ds, string dirname, string configname, bool updateconfig)
    {
        if (ds == null)
        {
            if (from != null && !from.Deleted)
            {
                from.SendMessage($"Empty dataset. File {dirname} not saved.");
            }
        ...
        if (from != null && !from.Deleted)
        {
            from.SendMessage($"Saved npc to file {dirname}");
        }
                ...
                default:
                    {
                        ...
                        {
                            state.Mobile.SendMessage($"File {m_filename} not saved.");
                        }
                        break;
                    }
            }
